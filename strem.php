...
class DoodStream {
    ...
    public function remoteUpload($directLinks, $fldId = null, $newTitle = null) {
        $results = [];
        foreach ($directLinks as $directLink) {
            if (preg_match('/https?:\/\/[^\s]+/', $directLink, $matches)) {
                $url = $matches[0];
                $results[] = [
                    'url' => $url,
                    'status' => 'Detected',
                    'message' => 'URL berhasil dideteksi.'
                ];
            } else {
                $results[] = [
                    'input' => $directLink,
                    'status' => 'Invalid',
                    'message' => 'Bukan URL yang valid.'
                ];
            }
        }
        return $results;
...
// Fungsi untuk menampilkan array secara rapi
function displayArray($data, $indent = 0) {
    $html = '';
    $indentStr = str_repeat('&nbsp;', $indent * 4);
    foreach ($data as $key => $value) {
        $html .= "<div style='margin-left: {$indent}em;'>";
        if (is_array($value)) {
            $html .= "<strong>{$key}:</strong><br>";
            $html .= displayArray($value, $indent + 1);
        } else {
    ...
    try {
        switch ($action) {
            case 'account_info':
                ...
                $message = "<div class='array-output'>" . displayArray($result) . "</div>";
                break;
            case 'list_files':
            ...
            case 'file_info':
                ...
                break;
            case 'remote_upload':
                $directLinks = array_filter(array_map('trim', explode("\n", $_POST['direct_links'])));
                $fldId = isset($_POST['fld_id']) ? trim($_POST['fld_id']) : null;
                $newTitle = isset($_POST['new_title']) ? trim($_POST['new_title']) : null;
                $result = $doodStream->remoteUpload($directLinks, $fldId, $newTitle);
                $message = "<div class='array-output'>" . displayArray($result) . "</div>";
                break;
            case 'create_folder':
    ...
    <script>
        ...
        actionSelect.addEventListener('change', function() {
            ...
            switch (this.value) {
                ...
                case 'remote_upload':
                    html = `
                        <label for="direct_links">Direct Links (satu per baris):</label>
                        <textarea name="direct_links" id="direct_links" rows="5" required></textarea>
                        <label for="fld_id">Folder ID:</label>
                        <input type="text" name="fld_id" id="fld_id">
